...
class GameAppInternal extends React.Component<GameApp.Props, GameApp.State> {
      ...
      <React.Fragment>
        <div className="canvas-container">
          ...
          {this.state.action === "pass" && (
            <div className="input">
              <div>Choose {this.state.isSecretButtonHeld ? "any number of" : "3"} cards to pass {directionText[this.state.snapshot.pass]}</div>
              <button onClick={this.handlePass} disabled={this.state.picks.length !== 3 && !this.state.isSecretButtonHeld}>
                {this.state.isSecretButtonHeld ? "Pass 'em all!" : "Pass"}
              </button>
            </div>
          )}
  ...
  private handlePass = () => {
    if (this.state.picks.length === 3 || this.state.isSecretButtonHeld) {
      this.stage.setAction("none", emptyArray(), true);
